...
[fq, qbar0] = charger(d .* 1e-9, z, [], 'Fuchs', opt);
tools.textheader();
...
load('li_v4_collkernel.mat');
qbar_li = [];
for ii=1:length(dvec)
    qbar_li(ii) = collkernel2charge(collkernel0{ii}, nit);
end
d_ranga = dvec;
fl_r = d_ranga <= 1e3;
...
[~, qbar_li2] =  charger(d .* 1e-9, z, [], 'Li', opt);
